...
namespace json {
    ...
    public:
        ...
        inline JSON() : Internal(), Type(Class::Null) {}

        inline JSON(initializer_list<JSON> list)
            : JSON()
        {
        ...
        inline static JSON Make(Class type) {
        ...
        inline Class JSONType() const { return Type; }

        /// Functions for getting primitives from the JSON object.
        inline bool IsNull() const { return Type == Class::Null; }

        inline string ToString() const { bool b; return std::move(ToString(b)); }
        inline string ToString(bool& ok) const {
            ok = (Type == Class::String);
            return ok ? std::move(json_escape(*Internal.String)) : string("");
        }

        inline double ToFloat() const { bool b; return ToFloat(b); }
        inline double ToFloat(bool& ok) const {
            ok = (Type == Class::Floating);
            return ok ? Internal.Float : 0.0;
        }

        inline long ToInt() const { bool b; return ToInt(b); }
        inline long ToInt(bool& ok) const {
            ok = (Type == Class::Integral);
            return ok ? Internal.Int : 0;
        }

        inline bool ToBool() const { bool b; return ToBool(b); }
        inline bool ToBool(bool& ok) const {
            ok = (Type == Class::Boolean);
            return ok ? Internal.Bool : false;
        }

        inline JSONWrapper<map<string, JSON>> ObjectRange() {
            if (Type == Class::Object)
                return JSONWrapper<map<string, JSON>>(Internal.Map);
            return JSONWrapper<map<string, JSON>>(nullptr);
        }

        inline JSONWrapper<deque<JSON>> ArrayRange() {
            if (Type == Class::Array)
                return JSONWrapper<deque<JSON>>(Internal.List);
            return JSONWrapper<deque<JSON>>(nullptr);
        }

        inline JSONConstWrapper<map<string, JSON>> ObjectRange() const {
            if (Type == Class::Object)
        ...
        inline JSONConstWrapper<deque<JSON>> ArrayRange() const {
            if (Type == Class::Array)
                return JSONConstWrapper<deque<JSON>>(Internal.List);
            return JSONConstWrapper<deque<JSON>>(nullptr);
        }

        inline string dump(int depth = 1, string tab = "  ") const {
            string pad = "";
